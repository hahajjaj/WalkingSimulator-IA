...
if __name__ == '__main__':
    ...
    selection_rates = np.linspace(0.1, 1, num_param, endpoint=False).tolist()
    # mutation_rate = np.random.rand(num_param).tolist()
    mutation_rates = np.linspace(0.01, 1, num_param, endpoint=False).tolist()


    parameters = list(itertools.product(selection_rates, mutation_rates))
    ...
    Y_best_scores = np.empty((gen_max, len(parameters)))
    for i, score in enumerate(scores):
        for j, gen_score in enumerate(score):
            Y_best_scores[j][i] = np.amax(gen_score)
    ...
    def on_move(event):
        if event.inaxes:
            for line, (sel, mut, best_score) in zip(lines, results):
                ...
                if cont:
                    print(f"sel: {sel:.4f} mut:{mut:.4f} best score: {best_score:.4f}")
