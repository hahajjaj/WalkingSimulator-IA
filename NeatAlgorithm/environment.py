...
class VectorizedEnv:
    ...
    def __init__(self, EnvClass: Type[IEnvironment], n_env: int, screen: Optional[pygame.Surface] = None, **kwargs):
        ...
        self.action_shape = (self.N_ENV, self.action_size)
...
class WalkingEnv:
    ...
    def __init__(self, time_max: int = 1000, sim_step: float = 1/10, screen: Optional[pygame.Surface] = None):
        self.info = {}
        ...
        if isinstance(self.screen, pygame.Surface):
            ...
            self.camera = HorizontalCamera()


    def _reset(self):
        CHASSIS_DIMS: tuple[float, float] = (50.0, 50.0)
        CHASSIS_INITIAL_POS: Vec2d = Vec2d(0.0, self.GROUND_Y - CHASSIS_DIMS[1] - 10)
        ...
        self.old_pos = self.get_position()
    ...
    def reset(self) -> tuple[np.ndarray, dict]:
        ...
        if self.screen:
            self.camera.reset(self.get_position())
    ...
    def get_obs(self) -> np.ndarray:
        obs = np.array([
            [
                ...
                *body.force,
                float(body.is_sleeping), # bool
                body.kinetic_energy,
                *body.position,
                *body.rotation_vector,
                body.torque,
                *body.velocity,
                len(shape.shapes_collide(self.ground).points)
            ]
            for body, shape in zip(self.bodies, self.shapes)
        ]).flatten()
        return np.array([*obs, self.time_step])
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
        ...
        animal_position = self.get_position()
    ...
    def render(self, _bg: bool = True):
        ...
        if _bg:
            self.camera.follow(self.get_position())
        
        camera_pos = self.camera.get_pos()

        if _bg:
            tiles = math.ceil(self.screen.get_width() / self.bg_width) + 1
            scroll = (camera_pos.x % self.bg_width) * (-1 if camera_pos.x > 0 else 1)

            for i in range(-1, tiles-1):
                self.screen.blit(self.background_image, (i * self.bg_width + scroll, 0))
            
        self.draw_options.transform = pymunk.Transform.translation(*camera_pos + Vec2d(300.0, 0.0))
...
class HorizontalCamera:
    ...
    def reset(self, point: Vec2d):
        self.x_pos = point.x

    def follow(self, point: Vec2d):
        heading = point.x - self.x_pos
        self.x_pos += heading * self.speed

    def get_pos(self) -> Vec2d:
        return -Vec2d(self.x_pos, 0.0)


class VectorizedWalkingEnv(VectorizedEnv):
    def __init__(self, EnvClass: Type[WalkingEnv], n_env: int, screen: Optional[pygame.Surface] = None, **kwargs):
        assert issubclass(EnvClass, WalkingEnv), TypeError(f"{self.__class__.__name__} only works with WalkingEnv environment")
        super().__init__(EnvClass, n_env, screen, **kwargs)
        
        self.camera = HorizontalCamera()

        for env in self.envs:
            env.camera = self.camera
...
class ConfigurableAnimalEnv(WalkingEnv):
    ...
    def __init__(self, config_animal: dict = _DEFAULT_CONFIG,
                  time_max: int = 1000, sim_step: float = 1/10, screen: Optional[pygame.Surface] = None):
        ...
        self.observation_size = len(self.config_animal['bones']) * 14 + 1 # position, angle, vélocité angulaire, torque/couple et vélocité de chaque bones + time_step
    ...
    def _reset(self):
        ...
        translation = Vec2d(0.0, self.GROUND_Y - 50) - (min_xy[0], max_xy[1])
    ...
    def get_position(self) -> Vec2d:
        return Vec2d(self._get_min_x(), self.GROUND_Y)
...
if  __name__ == '__main__':
    ...
    window = Window(EnvClass.display_size, EnvClass.render_fps, f"Exemple Vectorized {EnvClass.__name__} (fps: %fps%)")

    env = EnvClass(screen=window.screen) # screen peut être None -> pas de rendu (show == False)
...
if __name__ == '__main__':
    ...
    N_ENV = 20
    envs = VectorizedWalkingEnv(EnvClass, N_ENV, window.screen) # screen peut être None -> pas de rendu (show == False)

    obss, _ = envs.reset()
    dones = [False for _ in range(N_ENV)]
    scores = np.zeros(N_ENV)

    while not all(dones):

        obss, rewards, dones, _ = envs.step(np.random.uniform(-1, 1, envs.action_shape))
        scores += rewards
        
        # dessine les environnements sur le screen donné à la création de l'environnement, si None -> ne fait rien
        # peut décider de dessiner le background avec l'argument 'bg'
        # peut donné une list d'indexs des environments à dessiné dans l'ordre donné, par defaut tous dans l'ordre de creation
        envs.render(np.argsort(scores)[4::-1])
        # dessine info additionel si il faut (ex: gui)
        window.render()
